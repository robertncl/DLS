...
e.g. "Quarterly report"), red rule (`.acme-doc-rule`), title (4xl display),
and `.acme-doc-meta` (authors · date · classification).

**Body:** wrap in `.acme-doc`. `h2`/`h3` self-number via CSS counters
("3.", "3.1") with red numerals — don't hand-number headings. Body text is
16 px/1.65; captions and footers 12 px.

## Callouts

`.acme-callout` is the editorial sibling of the UI alert: surface background,
3 px red accent, uppercase label. Reserve it for **findings and decisions** —
