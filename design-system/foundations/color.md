...
only two colors that highlight it, and each has one job. Go and Caution appear
for status only. Components must reference **semantic tokens**
(`--acme-color-*`); primitive scales are for defining tokens, not for direct
use in product code.

## The two highlights

The base is grey. Everything that isn't grey is making a claim, and there are
...
| State | — | Selected rows, current page, checked controls, active tab |
| Editorial | Kickers, rules, heading numerals, callout accents | — |
| Data | The single takeaway mark | All other marks |
| Brand | The wordmark mark, deck section dividers | — |
...
checkboxes, radios, switches, and selected tabs are Blueprint, not red.
...
is not scarce. Together they should still read as accents on grey: if a
...
| Scale | Anchor | Role |
| --- | --- | --- |
| Graphite `--acme-gray-*` | `900 #16191F` | The base: text, surfaces, borders |
| ACME Red `--acme-red-*` | `600 #C8102E` | Highlight 1: brand, primary action, danger, editorial marks |
| Blueprint Blue `--acme-blue-*` | `600 #2563EB` | Highlight 2: links, selection, focus, info, data |
| Go Green `--acme-green-*` | `600 #15803D` | Success status |
| Caution Amber `--acme-amber-*` | `700 #B45309` | Warning status |
...
3. **Selection is never red.** Checked inputs, selected rows, the current
   page, and the active tab are Blueprint — they report state, not action.
4. **Meaning never travels by color alone.** Pair status color with an icon or
   label (WCAG 1.4.1); selected rows also carry `aria-selected`.
5. **Soft pairs stay together.** `*-soft` backgrounds take only their matching
   `*-soft-text` foreground; `selected-soft` takes `selected`.
6. **Focus is always Blueprint** at `--acme-color-focus`, never red and never
...
Red and blue are separable under all three common CVD types (they differ in
lightness as well as hue), but because deuteranopes see both as muted, the
system never relies on red-vs-blue alone: red carries a verb, blue carries a
position or an underline.
